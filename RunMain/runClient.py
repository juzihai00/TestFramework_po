...
import ctypes
from Base.baseGuiRun import run
...
def run_collect_testcase(v):
    with output_to_null():
        try:
            test_pj = os.path.join(BP.TEST_SUIT_DIR,run_config['TEST_PROJECT'])
...
def run_main():
    try:
        ...
        if run_config['REPORT_TYPE'] == 'ALLURE':
            ...
            report_path = os.path.join(BP.REPORTS_HTML_DIR, 'auto_reports.html')
            pytest.main(['-v', '--html={}'.format(report_path), '--self-contained-html', *testcases])
        elif run_config['REPORT_TYPE'] == 'XML':
            report_path = os.path.join(BP.REPORTS_XML_DIR, 'auto_reports.xml')
        ...
        if run_config['IS_EMAIL'] == 'yes':
            ...
            el.send_public_email(text=text, file_type=run_config['REPORT_TYPE'])
